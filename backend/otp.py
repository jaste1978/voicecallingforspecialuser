...
def issue(email: str) -> str | None:
    ...
    now = time.time()
    with _conn() as conn:
        row = conn.execute("SELECT sent_at FROM email_otps WHERE email = ?",
                           (email,)).fetchone()
...
def check(email: str, code: str) -> bool:
    ...
    now = time.time()
    with _conn() as conn:
        row = conn.execute("SELECT * FROM email_otps WHERE email = ?",
                           (email,)).fetchone()
        if not row or row["expires_at"] < now or row["attempts"] >= MAX_ATTEMPTS:
            return False
        if _hash((code or "").strip()) != row["code_hash"]:
            conn.execute("UPDATE email_otps SET attempts = attempts + 1"
                         " WHERE email = ?", (email,))
            return False
